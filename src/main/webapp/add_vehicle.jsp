...
<body background="download2.jpg">
	...
	<h3><center>Welcome to Warakagoda Tea Factory!!</center></h1>
			...
			<center>
			<br/>
				<form method = "post" action = "insert" onsubmit = "return validate()">
					<label><b>Vehicle Number: </b></label>
			   		<input type="text" placeholder="Vehicle Number" name="vnum" id = "vnum" required/>
			    	</br></br>
				    ...
				    <input type="text" placeholder="Manufactured Year" name="manf" id = "manf" required/>
			    	</br></br>
...
<footer>The website and its content is copyright of Warakagoda Estate -@ Warakagoda Estate 2019.All rights reserved.</footer>
	...
	<script type="text/javascript">
		function validate(){
			...
			var manf =  document.getElementById("manf").value;
			if(model.match(letters)){
				if(manf.match(numbers)){
					if(vnum.match(letterNumber)){
						if(vnum.length == 6 || vnum.length == 7){
							if(manf.length == 4){
								...
								alert("Warning: Manufactured Year field's length should be 4!!!");
								return false;
							}
						}
						else{
					...
					alert("Warning: Manufactured Year field should contain only numbers!!!");
					return false;	
				}
			}
			else{
				alert("Model field should contain only alphabets!!!");
				return false;
			}
		}
	</script>
